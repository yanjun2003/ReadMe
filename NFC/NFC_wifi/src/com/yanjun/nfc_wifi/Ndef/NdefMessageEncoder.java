...
public class NdefMessageEncoder {
	...
	public byte[] encode(List<Record> records) {
		...
		for (ListIterator<Record> it = records.listIterator(); it.hasNext();) {
			byte header = (byte)(NdefConstants.MB | NdefConstants.ME);
...
/*	private byte setMessageBeginIfFirstRecord(ListIterator<Record> it, byte header) {
		if (!it.hasPrevious()) {
	...
	private byte setMessageEndIfLastRecord(ListIterator<Record> it, byte header) {
		if (!it.hasNext()) {
